...
@bot.on_message(filters.command(["angel"]))
async def account_login(bot: Client, m: Message):
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            if "visionias" in url:
             ...
             id =  url.split("/")[-2]
             url =  "https://psitoffers.store/testkey.php?vid=" + vidId + "&quality=" +raw_text2
...
async def help_command_handler(_, callback_query):
    keyboard = [
        [
            InlineKeyboardButton("❤️ ᴜᴘɢʀᴀᴅᴇ ❤️", callback_data="upgrade_command"),
            InlineKeyboardButton("🍷ꜱᴜᴘᴘᴏʀᴛ🍷", url="https://t.me/LegendBotzs")
        ]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    reply_markup = InlineKeyboardMarkup(keyboard)
